...
import com.example.simplesudokuapp.Statistics
...
class LocalStatisticsStorageImpl(
    ...
    override suspend fun getStatistics(
        onSuccess: (com.example.simplesudokuapp.domain.UserStatistics) -> Unit,
        onError: (Exception) -> Unit,
    ) {
        try {
            val statistics = dataStore.data.first()
            onSuccess(statistics.toUserStatistics)
        }catch (e:Exception){
            onError(e)
        }
    }
